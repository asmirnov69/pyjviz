...
def run_ChainedMethodCall(cmc_name, cmc_head_obj):
    return Wrapper(cmc_head_obj)
...
if 0:
    ...
    df = (
        run_ChainedMethodCall("pyjanitor-helloworld", pd.DataFrame.from_dict(company_sales))
        .remove_columns(["Company1"])
        .dropna(subset=["Company2", "Company3"])
        .rename_column("Company2", "Amazon")
        .rename_column("Company3", "Facebook")
